...
class Reservations extends Model
{
    ...
    {
        $result = $this->db->dataQuery("SELECT MAX(id) AS lastResource FROM reservations");
        return $result[0]->ultimoIdLibro;
    }

    public function multiTable(){
        return $this->db->dataQuery("SELECT * FROM reservations 
                                    INNER JOIN resources ON idResources=resources.id 
                                    INNER JOIN timeSlots ON idTimeSlots=timeSlots.id
    ...
    {
        return $this->db->dataManipulation("INSERT INTO reservations (name,description,location,image) VALUES ('$name','$description', '$location', '$image')");
    }
    ...
    //         $correctos += $this->db->dataManipulation("INSERT INTO reservations(id) VALUES('$id')");
    //     }
    //     return $correctos;
    // }
